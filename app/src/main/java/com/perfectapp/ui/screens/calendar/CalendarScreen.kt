...
private enum class CalendarViewMode { AGENDA, MONTH, BIRTHDAYS, UNIVERSITY }
...
fun CalendarScreen(
    ...
    var birthdayQuery by remember { mutableStateOf("") }
    var pendingDelete by remember { mutableStateOf<com.perfectapp.data.entities.CalendarEventEntity?>(null) }

    Scaffold(
        floatingActionButton = {
            FloatingActionButton(onClick = if (mode == CalendarViewMode.BIRTHDAYS) onAddBirthday else if (mode == CalendarViewMode.UNIVERSITY) onAddUniversity else onAddEvent) {
            ...
            item {
                SingleChoiceSegmentedButtonRow(modifier = Modifier.fillMaxWidth()) {
                    ...
                    SegmentedButton(selected = mode == CalendarViewMode.BIRTHDAYS,
                        ...
                        shape = SegmentedButtonDefaults.itemShape(index = 2, count = 4)) { Text("Birthdays") }
                    SegmentedButton(selected = mode == CalendarViewMode.UNIVERSITY,
                        onClick = { mode = CalendarViewMode.UNIVERSITY },
                        shape = SegmentedButtonDefaults.itemShape(index = 3, count = 4)) { Text("University") }
                }
            }

            if (mode == CalendarViewMode.UNIVERSITY) {
                item { Text("Add lessons or exams here. Edit or remove lessons when your exam schedule changes.", style = MaterialTheme.typography.bodySmall) }
                val university = allEvents.filter { it.itemType == com.perfectapp.data.entities.CalendarItemType.UNIVERSITY_LESSON }.sortedBy { it.dateTime }
                if (university.isEmpty()) item { EmptyState("No university events", "Add a lesson or exam.", actionLabel = "Add University event", onActionClick = onAddUniversity) }
                items(university) { event -> EventRow(EventOccurrence(event, event.dateTime), { pendingDelete = event }, {}, { onEditEvent(event.id) }) }
            } else if (mode == CalendarViewMode.BIRTHDAYS) {
                item { androidx.compose.material3.OutlinedTextField(birthdayQuery, { birthdayQuery = it }, label = { Text("Search birthdays") }, singleLine = true, modifier = Modifier.fillMaxWidth()) }
                if (birthdays.isNotEmpty() && birthdays.none { it.event.title.contains(birthdayQuery, true) }) item { Text("No matching birthdays") }
                ...
                items(birthdays.filter { it.event.title.contains(birthdayQuery, true) }) { occurrence ->
    ...
    PremiumCard(modifier = Modifier.fillMaxWidth()) {
        Row(
            ...
            Column(Modifier.weight(1f)) {
                ...
                Text(occurrence.event.itemType.displayName + if (occurrence.event.isCompleted) " · completed" else "", style = MaterialTheme.typography.labelSmall)
                Text(
